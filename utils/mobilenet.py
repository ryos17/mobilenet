    ...
    def __init__(self, ch_in=3, n_classes=2, kernel_size=3, alpha=1.0):
        ...
        def conv_full(inp, oup, stride):
            ...
            return nn.Sequential(
                nn.Conv2d(inp, oup_scaled, kernel_size, stride, 1, bias=False),
        ...
        def conv_ds(inp, oup, stride):
            ...
            return nn.Sequential(
                ...
                nn.Conv2d(inp_scaled, inp_scaled, kernel_size, stride, 1, groups=inp_scaled, bias=False),
                nn.BatchNorm2d(inp_scaled),
                nn.ReLU(inplace=True),

                # Pointwise convolution
                nn.Conv2d(inp_scaled, oup_scaled, 1, 1, 0, bias=False),
                nn.BatchNorm2d(oup_scaled),
                nn.ReLU(inplace=True),
            )
